...
def test_render_slurm_script_uses_sweep_stem_and_relative_config_path(
    ...
    """
    repo_root = tmp_path
    sweep_path = repo_root / "experiments/configs/sweeps/example.toml"
    sweep_path.parent.mkdir(parents=True, exist_ok=True)
    sweep_path.write_text("name = 'example'\n", encoding="utf-8")

    rendered = render_slurm_script(
        SlurmJobConfig(
            sweep=Path("experiments/configs/sweeps/example.toml"),
            force=True,
        ),
        repo_root=repo_root,
...
def test_write_slurm_scripts_materialises_all_manifest_entries(tmp_path: Path) -> None:
    ...
    """
    repo_root = tmp_path
    sweep_root = repo_root / "experiments/configs/sweeps"
    ...
    manifest_path.write_text(
        (
            "sweeps = [\n"
            '  "experiments/configs/sweeps/first.toml",\n'
            '  "experiments/configs/sweeps/second.toml",\n'
            "]\n"
        ),
        encoding="utf-8",
    )
...
def test_load_slurm_jobs_rejects_duplicate_stems(tmp_path: Path) -> None:
    ...
    manifest_path.write_text(
        (
            "sweeps = [\n"
            '  "experiments/configs/sweeps/a.toml",\n'
            '  "experiments/configs/sweeps/nested/a.toml",\n'
            "]\n"
        ),
        encoding="utf-8",
    )

    with pytest.raises(ConfigError, match="repeat the same sweep stem"):
