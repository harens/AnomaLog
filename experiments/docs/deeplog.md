...
- `grouping = "chronological_stream"` gives BGL a deterministic entry-stream
  ...
  benchmark configs;
- chronological stream sequences can be marked continuous through the dataset
  sequence config, so DeepLog carries history across chunk boundaries when the
  stream contract asks for it;
